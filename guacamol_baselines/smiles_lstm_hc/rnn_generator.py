...
class SmilesRnnMoleculeGenerator:
    ...
    def optimise(self, objective: ScoringFunction, start_population, keep_top, n_epochs, mols_to_sample,
        ...
        for epoch in range(1, 1 + n_epochs):
            ...
            smiles_history.append([i.smiles for i in results][0:keep_top])
            

            to_keep = int(10 + (1-(epoch/n_epochs)) * keep_top)
            
            subset = [i.smiles for i in results][:keep_top]
            subset_scores = [i.score for i in results][:keep_top]
